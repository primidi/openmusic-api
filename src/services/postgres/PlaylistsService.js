...
class PlaylistsService {
  ...
  async addSongToPlaylist(playlistId, songId) {
    ...
    if (!result.rows.length) {
      throw new InvariantError('Song cannot be added in playlist.');
    }
  ...
  async getSongsFromPlaylist(playlistId) {
    ...
    const mappedPlaylist = result.rows.map(mapPlaylistDbToModel)[0];
  ...
  async deleteSongFromPlaylists(playlistId, songId) {
    ...
    if (!result.rows.length) {
      throw new InvariantError('Song cannot be deleted from playlist.');
    }
  }
  ...
  async newPlaylistActivity(playlistId, songId, userId, action) {
    ...
    if (!result.rows.length) {
      throw new InvariantError('Playlist activities cannot be logged.');
    }
  ...
  async verifyPlaylistOwner(id, owner) {
    ...
    if (playlist.owner !== owner) {
      throw new AuthorizationError('Sorry, you have no right to access this resource.');
    }
  }
